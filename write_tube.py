...
def sphere(point, r=1, k=10, j=10):
    x = np.array((1, 0, 0))
    vertex_rings = [
        vertex_ring(
            point + r * np.cos(theta) * x,
            x,
            r * np.sin(theta),
            k,
        )
        for theta in np.linspace(0, np.pi, j)
    ]
    return from_vertex_rings(vertex_rings, k)

def from_vertex_rings(vertex_rings, k=10):
    triangles = []
...
def tube(path, r=1, k=10, loop=True):
    vertex_rings = [
    ...
    if not loop:
        vertex_rings[0] = vertex_ring(path[0], path[1] - path[0], r, k)
        vertex_rings[-1] = vertex_ring(path[-1], path[-1] - path[-2], r, k)

    vertex_rings = vertex_rings + ([vertex_rings[0]] if loop else [])
